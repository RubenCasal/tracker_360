...
def wrap_x(x, pano_width=PANO_WIDTH):
    return x % pano_width

def unwrap_coord(new_x, ref_x, pano_width=PANO_WIDTH):
    if abs(new_x - ref_x) > pano_width / 2:
        if new_x < ref_x:
            new_x += pano_width
        else:
            new_x -= pano_width
...
class STrack(BaseTrack):
    ...
    def predict(self):
        ...
        self.mean[0] = wrap_x(self.mean[0])
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        new_xyah = self.convert_coords(new_track.tlwh)
        new_xyah[0] = unwrap_coord(new_xyah[0], self.mean[0])
        self.mean, self.covariance = self.kalman_filter.update(self.mean, self.covariance, new_xyah)
        self.mean[0] = wrap_x(self.mean[0])
...
class BYTETracker:
    ...
    def update(self, results, img=None):
        ...
        # TODO
        dists = matching.iou_distance_panoramic(r_tracked_stracks, detections_second, pano_width=1920)
    ...
    def get_dists(self, tracks, detections):
        ...
        dists = matching.iou_distance_panoramic(tracks, detections,pano_width=1920)
    ...
    def remove_duplicate_stracks(stracksa, stracksb):
        ...
        pdist = matching.iou_distance_panoramic(stracksa, stracksb,pano_width=1920)
